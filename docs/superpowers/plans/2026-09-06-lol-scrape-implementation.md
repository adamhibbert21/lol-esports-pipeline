...
9. **Failed games need carry-forward too (discovered during live verification of Task 9's fix, 2026-09-07).** After Task 9 landed, live verification of the cross-day carry-forward mechanism (running the notebook twice same-day, per the user's request to confirm future runs only scrape new matches) surfaced that a game which fails to parse was never added to `already_fetched_ids` (only `known_rows`, sourced from `games.csv`, fed it), so a permanent parse failure (not a transient network error - a real gol.gg page defect) got re-fetched and re-appended to `failures.csv` on every subsequent run, growing an ever-duplicating file and wasting a live request per run forever on something that will never succeed. Fix: `failures.csv` now carries forward from the latest snapshot exactly like `games.csv` does, and its game_ids join `games.csv`'s into `already_fetched_ids`, so a previously-attempted game (success or failure) is never retried. Verified live: two consecutive same-day runs both correctly report "no changes" for all 4 regions, and `failures.csv` stays at one row per real failure instead of accumulating duplicates.

10. **Second final-review pass findings, all fixed (2026-09-07).** A re-scoped final whole-branch review (covering Task 9 plus the two live-verification follow-ups above) found I3 (global-lists staleness guard) had NOT actually been fixed despite being recorded as fixed, plus one real bug in the failures-carry-forward fix (item 9 above: it treated every past failure, including transient network failures, as permanently handled) and one more instance of the C1 contamination class in `players.csv`/`champions.csv` (still globally-fetched, split-filtered only, not tournament-filtered - measured 1371 global player rows vs ~160 for the 4 target leagues, and LCK/LPL don't even use "Summer" as a split name). User decision: narrow players/champions to the real `TOURNAMENTS` map too, same mechanism as C1, confirmed live to work for these entities (LCK 2026 Rounds 3-4 -> 55 players, not 1371). Fixed together with several Minor findings from the same review round: global-lists guard now requires both files exist before comparing; failures carry-forward now excludes transient fetch failures (identified by `fetch_html`'s error always starting with "Failed to fetch") from the permanent-skip set; `teams.csv`'s gol.gg-native `Region` column renamed to `golgg_region` to avoid colliding with the new `region` column; `games.csv` writes (both the carry-forward header and `append_row`) now use `GAMES_ROW_COLUMNS` explicitly as CSV fieldnames rather than a dict's own key order, closing a narrower version of the schema-corruption path where a pure column reorder could pass the earlier guard's set-equality check; `games_added` no longer crashes when a region has no prior snapshot and discovers zero games; `teams_changed` now captures the prior snapshot's hash before writing today's file, instead of comparing the file against itself on a same-day rerun; `hash_table`'s row-sort key fix (item 7) was completed (it previously sorted by the pre-column-sort order); the missing `KogMaw` champion-name casefold entry was added; print statements show a path relative to the project root instead of an absolute local path; `TEST_MODE` restored to `True` as the committed default, matching the intro markdown's own advice. Verified live: a full 4-region backfill (10/10/8/12 teams, 99/106/63/166 games) followed by a TEST_MODE run, both correct.

11. **games.csv had no way to identify a game's position within its Bo1/Bo3/Bo5 series, or to group a series' games together (found post-merge, 2026-09-07).** A user question ("some games are best of 3 or 5, do you mean the individual games or the series?") surfaced that `game_id` is one individual game (map), confirmed via gol.gg's own page title format ("ANB vs Disruptors game 3 - ..."), but nothing in the schema recorded which game number that was or linked it to its series. Fix: `parse_game_meta` now also extracts `game_number_in_series` from the page `<title>` (raises loudly if the pattern isn't found, consistent with this project's fail-loud philosophy); `assemble_game_row` derives `series_id` from the two team names (sorted, since `team_1`/`team_2` is page order not stable per-team identity) plus the date, since gol.gg has no single ID for a series and the two teams + date are what every game in the same series shares. Both are now the first two columns after `game_id` in `GAMES_ROW_COLUMNS`. Verified against the real fixture (game 80757 -> `game_number_in_series=3`, `series_id="ANB|Disruptors_2026-08-05"`), not yet verified against a fresh live scrape - deliberately deferred at the user's request to avoid the ~15-20 minute full 4-region run; the next live run of `lol_scrape.ipynb` (full or TEST_MODE) will exercise this for real and should be spot-checked before trusting the field broadly (e.g. confirm a real Bo3/Bo5 series' games all share one `series_id` and get consecutive `game_number_in_series` values).
